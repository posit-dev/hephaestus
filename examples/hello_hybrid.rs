//! The `hello` scene through the sparse-strips backend, for eyeballing the two
//! against each other. Writes `examples/hello_hybrid.png` in the project root;
//! `examples/hello.png` is the compute-shader backend's version of the same
//! drawing.
...
fn main() {
    let mut renderer = HybridRenderer::new().expect("hybrid renderer init");
