...
//! ```
//!
//! Two things bound how far that goes. The compute-shader backend rasterises
//! at most [`MAX_DRAW_INFO_WORDS`] flat-coloured objects per scene, and a
//! per-row `"fill"` makes every point one of them — past that the render
//! returns `BackendError::SceneTooLarge` and `run` exits with it. Well before
//! that, hovering dominates: every crossing rebuilds all five channels for all
//! rows.
//!
...
/// Resolve the backend named on the command line.
///
/// `hybrid` only selects in a build that compiled it in, so an unavailable
/// name is reported rather than silently falling back to the other backend.
