...
- **`Track`** — per-row / per-column sizing. `Track::Fr(f64)` (fraction unit), `Track::Auto` (size to content), or `Track::Fixed(Extent)`.
- **`Extent`** — `Sum { px, inches, percent }` (the common linear combination), plus `Min(Box, Box)` / `Max(Box, Box)` for deferred pointwise resolution, plus `TrackOf { grid, axis, track, span }` for cross-grid references that resolve at solve time.
...
- **`Axis`** — `Width` / `Height`. Selects which axis of a target grid a `Extent::TrackOf` references.
...
Iteration kicks in when any cell returns `WidthHint::NeedsHeight { seed }` **or** when any `Extent::TrackOf` reference appears in the tree.
...
`Extent::TrackOf` references resolve to 0 on iteration 0; on later iterations they pick up the resolved track size from the previous iteration. Forward references converge in 1–2 iterations; cycles will not converge and exhaust `MAX_ITER`.
...
- **`Extent` is the layout measurement; `style_vocab::Length` is the typographic one.** They are different vocabularies — `Extent` resolves to pixels against a dpi and can reference another grid's track; `Length` is a pt value that may be relative to a parent font size. The names are distinct so `hephaestus::Length` can mean the one users write in themes.
- **`Extent`, `Track`, `Inset` are `Clone` but not `Copy`** because of the `Box`es in `Min`/`Max`. The solver pattern-matches by reference and uses `Option::as_ref()` on `Inset` fields.
- **Constructors `px` / `mm` / `cm` / `inch` / `pt` / `percent`** produce a `Sum`. `Extent::min` / `Extent::max` produce `Min` / `Max`. `Extent::track_of` / `Extent::tracks_of` produce `TrackOf`.
...
- **Resolution helpers** — `length_to_px(&Extent, dpi, axis)` walks the tree; `length_to_px_abs(&Extent, dpi)` treats the percent term as 0 (used for intrinsic-min computation).
...
- `composition/` — builds on top of `layout`. The patchwork chrome-mirroring mechanism is exactly `Extent::TrackOf` references between nested `Grid`s.
