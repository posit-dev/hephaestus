...
- **A document on disk is `.hep`.** Nothing in the crate reads or writes the extension — every entry point takes bytes or a caller-chosen path — so it is a convention the examples, the test fixture and the wasm demo keep. The bytes identify themselves by the `HEPHPLOT` magic, which is a separate thing from what the file is called.
