...
- **`CompositionError`** — every way a composition can be malformed: a degenerate extent, a placement off the grid, a mismatched track list, an append onto a multi-track composition, a `grid` cell-count mismatch, and duplicate patch ids. `PanelSlot` and `PanelCovered` are inert variants no builder produces, kept so existing exhaustive matches compile.
...
- **`Composition::slot` accepts `Slot::Panel`**, and `place_at` may cover the panel cell. The facets occupy those tracks, so the rect resolves to the band they fill — usable for a shared panel background or an overlay. The panel row and column are `Fr`, so a measure there sizes nothing; only a span reaching into the surrounding Auto chrome tracks affects the solve, as any spanning placement does.
