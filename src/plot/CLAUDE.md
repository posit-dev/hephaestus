...
The canonical user-facing surface is **`PlotComposition`** (in `composition.rs`). It owns the layout shape, the scale registry, and a `HashMap<String, Plot>` of attached plots. `view.render(scene, size, dpi)` is the single entry point: the orchestrator rebuilds the composition fresh on every render with each plot's chrome wired into anatomical slots, solves the layout, and drives each plot's draw passes in the order the layering rules ask for.
...
- **`Plot`** (`plot.rs`) — bound to a patch id. Stores channel → scale-name bindings, geom list (`Vec<(GeomId, Box<dyn Geom>)>`), chrome text (title / subtitle / caption / axis titles), a `ShapeRegistry` and an `ImageRegistry`. Lifecycle methods used by the orchestrator: `wire(patch, registry, dpi)` (drop chrome cells + panel into named slots; full version is `text`-gated, `wire_panel` is always available), then the draw passes — `draw_patch_background_into`, `draw_panel_chrome_into`, `draw_frame_into`, `draw_geoms_into`, `draw_labels_into`. `draw_into` runs all five in order.
...
- **`ImageRegistry`** (re-exported from `crate::image_registry`) — the name → `brush::Image` map `ImageGeom` resolves its `"image"` channel against, and markdown `![](…)` tags with it. At the crate root rather than under `plot/` because `text::rich` resolves image tags while shaping and must not import from `plot/` — the same reason `linetype` and `style_vocab` sit there. `plot::ImageRegistry` and `plot::image_registry` still resolve.

  `Plot` owns one behind an `Arc` (`image_registry` / `image_registry_ref` / `image_registry_mut` / `set_image_registry`) and threads it into `GeomContext::images` **and** into every markdown chrome slot — break labels, strip labels, titles, axis titles, legend text. `PlotComposition` owns a second one with the same four accessors, for its own chrome; a plot's chrome reads its plot's register and the composition's reads the composition's, with no fallback between them. `GeomContext::new` defaults to `ImageRegistry::shared_empty()`, so a context nobody handed a register to still resolves locations but finds no registered names.

  A name the register does not hold is read as a location and cached there, so `resolve` (not `get`) is what a draw path calls. That is also what the document writer embeds, which is how a figure whose title holds `![](logo.png)` survives being rebuilt in a browser.
...
- **Composition chrome draws after the per-plot chrome** (render phase 4). It shares canonical rows with the border facets' own chrome — see `build_wrapped_composition` — so the composition's wider rect paints over the band the narrower per-facet chrome sits in. Both go down before the geoms; see the draw-order convention below.
...
- **Chrome layers against the data by what it is, in three groups.** `PlotComposition::render` runs seven passes, each across every attached plot before the next begins, so one plot's background can never overpaint another's spilled geoms:

  | | pass | contents |
  |---|---|---|
  | under, always | 1–2 | patch background; panel background + grid |
  | frame | 3 *or* 5 | panel outline, axes, facet strips — pass 3 when the plot doesn't clip, pass 5 when it does |
  | geoms | 4 | |
  | over, always | 6–7 | legends (side and in-panel), title / subtitle / caption, axis titles; then the composition's own chrome |

  - **The frame follows `Plot::is_clipped` because the panel outline is the very line a clipped mark is cut against.** Clipped, a mark on the boundary ends in a straight cut flush with the outline; drawn under it, that cut edge lies half across the outline and the border reads as chewed. Stroking the outline last fixes it. Unclipped, the opposite: a mark is *meant* to cross the boundary, and an outline over it would slice it and make it look clipped after all. Axes and strips ride along with the outline they border.
  - **Legends and titles are always over.** A label exists to be read, and an in-panel legend carries an opaque background precisely so it can sit on the data. Composition chrome is entirely labels, and draws after the per-plot ones so a shared title paints over the canonical band it shares with the border facets'.
  - **One thing doesn't fit the split: a polar axis title.** Placing it needs the label extents the polar axis pass measures, so `chrome::polar` draws it inline and it lands in the frame rather than with the other titles. It sits out past the label rail, beyond anything a clipped geom can reach, so the layer it takes never shows.
  - **`Plot::draw_frame_into` and `draw_labels_into`** are the two halves the orchestrator phases apart. `Plot::draw_into` runs a whole plot's passes in the right order and is what a stand-alone (non-orchestrator) caller wants.
...
- **Chrome text is measured at the width it will be drawn at.** A measure that lays its run out unconstrained while the draw pass wraps reserves one line for an N-line block, and the surplus lines get clipped. Text that wraps at draw time either goes through `Cell::measured(run)` (the solver re-breaks via `Measure::height_at`) or re-breaks inside its own `Measure` at the same width, minus the same padding and text margin, that the draw pass uses — `chrome::strip::StripMeasure` is the worked example, including `WidthHint::NeedsHeight` for the rotated case where the thickness depends on the strip's length. Text that draws on one line (tick labels, rotated axis titles) anchors on the run's natural width, never on `Measure::width_hint` — the hint is the longest unbreakable cluster, a wrap lower bound that undershoots any label containing a space.
- **Unwrapped chrome shapes through `chrome::text::ChromeRun`.** Break labels, legend and colorbar titles, polar labels and legend text swatches all shape at natural width and never re-break. `ChromeRun` is the one type both the measure and the draw pass hold, so a slot can't reserve a box shaped one way and paint one shaped the other — the failure mode that made the two drift when each site built its own `TextRun`. A slot opts into markdown by carrying a `RichChrome` (built once per slot by `rich_chrome_for`, so every label it shapes shares one sheet `Arc`); `None` shapes plain text and keeps the separate outline pass.
  - **The shaped-run cache is thread-local.** Every public chrome entry point (`axis::measure`, `legend_measure`, `render_legend_stack`, …) is API a host calls, so threading a cache reference through them would break those signatures. `RichKey` already covers everything that decides what a run looks like, so one cache per thread only lets plots dedupe labels they share. Rendering is single-threaded by design, which is what keeps `Rc` sound.
- **The theme's root `text` element reaches every text slot, legends included.** `Theme::resolved_axis` folds it in through `PerAxis::resolve_with_root`; `chrome::legend::legend_text_elements` takes it explicitly and `AxisTheme::resolved_with_root` carries it into a legend's break-label cascade. A figure-wide font, colour or `markdown` switch set once on `theme.text` is expected to land on a legend title the same way it lands on an axis title.
