...
- **One backend per subfolder.** Each backend lives in `src/backend/<name>/` with at minimum `mod.rs` (the `SceneBuilder` and `Renderer` impls).
...
4. Map our restricted enums (`FillRule`, `BlendMode`, `Compose`, `Mix`, `Sampling`) to the backend's native types. `backend/convert.rs` is the reference and, for a peniko backend, the implementation.
