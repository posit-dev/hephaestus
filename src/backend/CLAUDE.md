...
- **One backend per subfolder.** Each backend lives in `src/backend/<name>/` with at minimum `mod.rs`. A *rasterising* backend implements `SceneBuilder` and `Renderer`; a *vector* backend implements `SceneBuilder` alone and exposes its own emit entry points, because `Renderer`'s contract is to fill a buffer of RGBA8 and there are no pixels to fill. `backend/svg/` is the worked example.
- **`mesh.rs` is shared by every backend, including the ones with no GPU.** Mesh decomposition works purely in this crate's own types and emits plain `SceneBuilder::fill` calls, so `backend/svg/` reuses it verbatim; its cfg gate names `svg` alongside the rasterisers for that reason. `convert.rs` does not, because it maps onto *peniko's* enums.
...
- **Feature-gated.** Each backend is gated by a cargo feature of the same name (`vello`, `svg`, future `blend2d`). `vello` and `png` are default-on; `jpeg` / `tiff` / `webp` gate the other writers in `src/image/`. `blend2d` and `pdf` are stub features (no code behind them yet) so dependent crates can write `features = ["blend2d"]` once available.
