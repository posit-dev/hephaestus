...
/// [`write_png_to`] for how `dpi` is treated.
pub fn encode_png(width: u32, height: u32, pixels: &[u8], dpi: Option<f64>) -> io::Result<Vec<u8>> {
    let mut out = Vec::new();
    write_png_to(&mut out, width, height, pixels, dpi)?;
    Ok(out)
}
...
/// `path` as a PNG.
///
/// See [`write_png_to`] for how `dpi` is treated.
pub fn write_png(
    ...
    write_png_to(BufWriter::new(file), width, height, pixels, dpi)
}
...
mod tests {
    ...
    fn encode_png_starts_with_the_png_signature() {
        let bytes = encode_png(4, 3, &checkerboard(4, 3), None).expect("encode");
        assert!(
    ...
    fn encode_png_and_write_png_agree_byte_for_byte() {
        ...
        let in_memory = encode_png(5, 4, &pixels, None).expect("encode");

        let dir = std::env::temp_dir().join("hephaestus_png_roundtrip");
        std::fs::create_dir_all(&dir).expect("create temp dir");
        let path = dir.join("roundtrip.png");
        write_png(&path, 5, 4, &pixels, None).expect("write");
    ...
    fn pixels_including_alpha_round_trip_exactly() {
        ...
        let bytes = encode_png(16, 9, &pixels, None).expect("encode");
    ...
    fn a_dpi_is_recorded_as_pixels_per_metre() {
        let bytes = encode_png(4, 3, &checkerboard(4, 3), Some(192.0)).expect("encode");
    ...
    fn no_dpi_writes_no_phys_chunk() {
        let without = encode_png(4, 3, &checkerboard(4, 3), None).expect("encode");
    ...
    fn an_unusable_dpi_falls_back_to_the_floor() {
        for dpi in [0.0, -96.0, f64::NAN, f64::INFINITY] {
            let bytes = encode_png(2, 2, &checkerboard(2, 2), Some(dpi)).expect("encode");
            let chunk = find_chunk(&bytes, b"pHYs").expect("pHYs chunk");
    ...
    fn wrong_length_buffer_is_rejected() {
        ...
        let err = encode_png(4, 3, &short, None).expect_err("short buffer must fail");
        assert_eq!(err.kind(), io::ErrorKind::InvalidInput);

        let mut sink = Vec::new();
        let err = write_png_to(&mut sink, 4, 3, &short, None).expect_err("short buffer must fail");
        assert_eq!(err.kind(), io::ErrorKind::InvalidInput);
