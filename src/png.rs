...
pub use crate::image::{
    decode_png, encode_png, read_png, read_png_from, write_png, write_png_to, PngCompression,
};
...
mod tests {
    ...
    fn png_entry_points_are_reachable_from_this_module() {
        use crate::png::PngCompression;
        let pixels = vec![255u8; 2 * 2 * 4];
        let bytes =
            crate::png::encode_png(2, 2, &pixels, PngCompression::Balanced, None).expect("encode");
        assert_eq!(
            &bytes[..8],
